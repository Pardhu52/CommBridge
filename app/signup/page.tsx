...
import { ArrowLeft, Users, Loader2, Mail, Phone } from "lucide-react"
...
export default function SignUpPage() {
  ...
  const handleEmailSignUp = async () => {
    ...
    try {
      ...
      setTimeout(() => { window.location.href = "/community-setup"; }, 2000);
    } catch (err: any) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!acceptedTerms) return setError("Please accept the terms and conditions.");
      ...
      <div className="w-full max-w-md">
        ...
        <Card>
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl">{step === 'otp' ? 'Verify Your Number' : 'Join CommBridge'}</CardTitle>
            <CardDescription>
              {step === 'otp' ? `We sent a code to ${phoneNumber}` : 'Connect with your community in seconds'}
            </CardDescription>
          </CardHeader>

          <CardContent>
            {error && <Alert variant="destructive" className="mb-4"><AlertDescription>{error}</AlertDescription></Alert>}
            {success && <Alert className="mb-4 bg-green-50 border-green-200 text-green-800"><AlertDescription>{success}</AlertDescription></Alert>}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-start space-x-2 pt-2">
                <Checkbox id="terms" checked={acceptedTerms} onCheckedChange={(checked) => setAcceptedTerms(checked as boolean)} disabled={loading}/>
                <div className="grid gap-1.5 leading-none">
                  <Label htmlFor="terms" className="text-sm font-normal">I agree to the <Link href="/terms" className="text-primary hover:underline">Terms of Service</Link></Label>
                </div>
              </div>

              <Button type="submit" className="w-full" disabled={loading || !acceptedTerms}>
                {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Users className="mr-2 h-4 w-4" />}
                {loading ? 'Processing...' : (step === 'otp' ? 'Verify & Create Account' : 'Continue')}
              </Button>
